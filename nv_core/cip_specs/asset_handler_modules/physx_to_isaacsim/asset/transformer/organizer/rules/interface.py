...
class InterfaceConnectionRule(RuleInterface):
    ...
    def get_configuration_parameters(self) -> List[RuleConfigurationParam]:
        ...
        return [
            ...
            RuleConfigurationParam(
                ...
                "Adds target_path to asset_path. Sublayer adds as sublayer; Reference/Payload/Inherit add to default prim.",
                default_value=[],
            ),
            RuleConfigurationParam(
    ...
    def process_rule(self) -> Optional[str]:
        ...
        if not interface_asset_name:
            ...
            if input_stage_path:
                ...
                interface_asset_name = base_name + ".usd"
            else:
                interface_asset_name = "asset.usd"
        # Ensure the name has a valid USD extension
        elif not interface_asset_name.endswith((".usd", ".usda", ".usdc")):
            interface_asset_name = interface_asset_name + ".usd"
        ...
        interface_layer_path = os.path.join(self.package_root, interface_asset_name).replace("\\", "/")
        interface_layer = self._create_or_open_layer(interface_layer_path)
        if not interface_layer:
            self.log_operation(f"Failed to create interface layer at {interface_layer_path}")
            return None

        # Copy stage metadata (metersPerUnit, upAxis) from source stage
        utils.copy_stage_metadata(self.source_stage, interface_layer)
        self.log_operation("Copied stage metadata (metersPerUnit, upAxis) to interface layer")

        # Get or determine default prim path
        default_prim_path = self._get_default_prim_path()
    ...
    def _create_or_open_layer(self, layer_path: str) -> Optional[Sdf.Layer]:
        """Create a new layer or open existing one.

        Args:
            layer_path: Absolute path to the layer.

        Returns:
            The layer object or None if creation failed.
        """
    ...
    def _get_default_prim_path(self) -> str:
        """Get the default prim path from source stage.
    ...
    def _add_connection(
        ...
        elif connection_type == CONNECTION_INHERIT:
            inherit_path = Sdf.Path(asset_path)
            if prepend:
                prim_spec.inheritPathList.Prepend(inherit_path)
            else:
                prim_spec.inheritPathList.Append(inherit_path)
    ...
    def _apply_custom_connections(
        ...
        For Sublayer, adds target_path as a sublayer to asset_path.
        For Reference/Payload/Inherit, adds target_path to the default prim on asset_path.

        Args:
            interface_layer: The interface layer (unused, kept for API compatibility).
        ...
        for spec in connections:
            ...
            if not asset_path or not target_path:
                self.log_operation(f"Missing asset_path or target_path in connection spec: {spec}, skipping")
                continue

            # Validate connection type
            if connection_type not in VALID_CONNECTION_TYPES:
                self.log_operation(f"Invalid connection type '{connection_type}' for {asset_path}, skipping")
                continue

            # Check if both asset layer and target path exist
            ...
            # Open the asset layer (the layer being modified)
            ...
            self.log_operation(f"Added connection: {target_path} -> {asset_path} via {connection_type}")
