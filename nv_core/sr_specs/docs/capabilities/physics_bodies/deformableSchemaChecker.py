...
# Physics attribute constants
YOUNGS_MODULUS_ATTR = "omniphysics:youngsModulus"
MASS_ATTR = "physics:mass"
REST_SHAPE_POINTS_ATTR = "omniphysics:restShapePoints"
SOLVER_POSITION_ITERATION_COUNT_ATTR = "physxDeformableBody:solverPositionIterationCount"
MAX_DEPENETRATION_VELOCITY_ATTR = "physxDeformableBody:maxDepenetrationVelocity"
SRC0_ATTR = "omniphysics:src0"
...
def _strip_api_from_spec(spec: Sdf.PrimSpec, schema: str):
    ...
    if src_list_op.isExplicit:
        ...
        if explicit:
            dst_list_op.explicitItems = explicit  # base list
        if prepended:
            dst_list_op.prependedItems = prepended
        if appended:
            dst_list_op.appendedItems = appended
        if added:
            dst_list_op.addedItems = added
        if deleted:
            dst_list_op.deletedItems = deleted
...
def _convert_rel(
    old_token: str, new_token: str, old_prim: Usd.Prim, new_prim: Usd.Prim, target_map: Optional[Dict] = None
):
    old_rel = old_prim.GetRelationship(old_token)
    if not old_rel:
        return
...
def _add_bindpose(point_based: UsdGeom.PointBased, rest_points: Vt.Vec3fArray) -> None:
    ...
    purposes = ["bindPose"]
    point_based.GetPrim().CreateAttribute(
        "deformablePose:default:omniphysics:points", Sdf.ValueTypeNames.Point3fArray
    ).Set(rest_points)
    point_based.GetPrim().CreateAttribute(
...
def _weld_tri_mesh(
    points: Vt.Vec3fArray, rest_points: Vt.Vec3fArray, sim_indices: Vt.IntArray
    ...
    return (type(points)(new_points), type(rest_points)(new_rest), type(sim_indices)(new_sim))


def _find_surface_triangle_indices_for_tets(
    tetVertexIndices: list[Gf.Vec4i], surfaceVertexIndices: list[Gf.Vec3i], tetIndices: list[int]
) -> list[int]:
    """
    ...
    tri_set_to_index = {frozenset((tri[0], tri[1], tri[2])): i for i, tri in enumerate(surfaceVertexIndices)}
...
def _find_triangle_indices_for_vertices(triVertexIndices: list[Gf.Vec3i], vertexIndices: list[int]) -> list[int]:
    """
...
class DeformableSchemaChecker(BaseRuleChecker):
    ...
    @staticmethod
    def retype_mesh_to_xform(stage: Usd.Stage, prim_path: Sdf.Path) -> Usd.Prim:
    ...
    def convert_PhysxPBDMaterial(stage: Usd.Stage, path: Sdf.Path):
        ...
        _convert_attr("nonexistent", YOUNGS_MODULUS_ATTR, prim, prim, 10000000.0 * mpu / kpu)
    ...
    def convert_PhysxDeformableSurfaceMaterial(stage: Usd.Stage, path: Sdf.Path):
        ...
        _convert_attr(
            "physxDeformableSurfaceMaterial:thickness", "omniphysics:surfaceThickness", prim, prim, 0.005 / mpu
        )
    ...
    def convert_PhysxDeformableBodyMaterial(stage: Usd.Stage, path: Sdf.Path):
        ...
        _convert_attr(
            "physxDeformableBodyMaterial:youngsModulus", YOUNGS_MODULUS_ATTR, prim, prim, 50000000.0 * mpu / kpu
        )
        _convert_attr(
    ...
    def clean_old_deformable_apis_and_attrs(deformable_prim: Usd.Prim):
        ...
        prefixes = ["physxAutoParticleCloth:", "physxDeformable:", "physxParticle:", "physxCollision:", MASS_ATTR]
    ...
    def convert_PhysxParticleCloth(stage: Usd.Stage, path: Sdf.Path):
        ...
        _convert_attr(MASS_ATTR, "omniphysics:mass", prim, prim)
        ...
        if len(welded_triangle_indices) > 0:
            if (
                len(welded_triangle_indices) % 3 == 0
                and len(welded_vertices_remap_to_orig) > 0
                and len(welded_vertices_remap_to_orig) < len(points)
            ):
            ...
            sim_mesh_tri_vtx_counts, sim_mesh_tri_vtx_indices = _triangulate_mesh(
            ...
            sim_mesh_points, sim_mesh_rest_points, sim_mesh_tri_vtx_indices = _weld_tri_mesh(
                points, rest_points, sim_mesh_tri_vtx_indices
            )
        ...
        sim_mesh.GetPrim().GetAttribute(REST_SHAPE_POINTS_ATTR).Set(sim_mesh_rest_points)
        ...
        if particle_system:
            ...
            _convert_attr(
                ...
                SOLVER_POSITION_ITERATION_COUNT_ATTR,
                particle_system_prim,
                prim,
            )
            _convert_attr("maxDepenetrationVelocity", MAX_DEPENETRATION_VELOCITY_ATTR, particle_system_prim, prim)
    ...
    def convert_PhysxDeformableSurface(stage: Usd.Stage, path: Sdf.Path):
        ...
        _convert_attr("physxDeformable:maxDepenetrationVelocity", MAX_DEPENETRATION_VELOCITY_ATTR, prim, prim)
        _convert_attr("physxDeformable:selfCollision", "physxDeformableBody:selfCollision", prim, prim)
        _convert_attr(
            "physxDeformable:selfCollisionFilterDistance", "physxDeformableBody:selfCollisionFilterDistance", prim, prim
        )
        ...
        rest_bend_angles_default = (
            "restShapeDefault" if flatteningEnabled is None or not flatteningEnabled else "flatDefault"
        )
        sim_mesh.GetPrim().CreateAttribute("omniphysics:restBendAnglesDefault", Sdf.ValueTypeNames.Token).Set(
            rest_bend_angles_default
        )
        ...
        sim_mesh.GetPrim().ApplyAPI("OmniPhysicsSurfaceDeformableSimAPI")
        sim_mesh.GetPrim().CreateAttribute(REST_SHAPE_POINTS_ATTR, Sdf.ValueTypeNames.Point3fArray).Set(rest_points)
        sim_mesh.GetPrim().CreateAttribute("omniphysics:restTriVtxIndices", Sdf.ValueTypeNames.Int3Array).Set(
            sim_mesh_tri_vtx_indices
        )
    ...
    def convert_PhysxDeformableBody(stage: Usd.Stage, path: Sdf.Path):
        ...
        sim_mesh.GetPrim().GetAttribute(REST_SHAPE_POINTS_ATTR).Set(sim_rest_points)
    ...
    def convert_DeformableTriMeshRigidAttachment(
        prim: Usd.Prim, deformable_prim: Usd.Prim, deformableSlot: int, xformable: UsdGeom.Xformable
    ) -> None:
        """
        ...
        _convert_rel(f"actor{deformableSlot}", SRC0_ATTR, prim, new_prim, DeformableSchemaChecker.root_to_sim_path_map)
        ...
        if rigid_rel != Sdf.Path():
            ...
            new_element_collision_filter = stage.DefinePrim(
                new_element_collision_filter_path, "OmniPhysicsElementCollisionFilter"
            )
            ...
            deformable_tri_indices = _find_triangle_indices_for_vertices(
                face_vertex_indices3, deformable_filter_indices
            )

            new_element_collision_filter.GetPrim().GetAttribute("omniphysics:groupElemCounts0").Set(
                [len(deformable_tri_indices)]
            )
            new_element_collision_filter.GetPrim().GetAttribute("omniphysics:groupElemIndices0").Set(
                deformable_tri_indices
            )
    ...
    def convert_DeformableBodyRigidAttachment(
        ...
        tet_finder_sim = DeformableSchemaChecker.physx_attachment.create_tet_finder(
            sim_mesh_bind_points, sim_mesh_indices
        )
        output = DeformableSchemaChecker.physx_attachment.points_to_tetmesh_local(
            tet_finder_sim, deformable_sim_attachment_points
        )
        ...
        if len(sim_mesh_tet_ids) != len(deformable_sim_attachment_points) or len(sim_mesh_barycentric_coords) != len(
            deformable_sim_attachment_points
        ):
            return
        ...
        triples = [
            (i, b[:3], r)
            for i, b, r in zip(sim_mesh_tet_ids, sim_mesh_barycentric_coords, rigid_attachment_points)
            if i != -1
        ]
        if len(triples) > 0:
    ...
    def convert_PhysxPhysicsAttachment(stage: Usd.Stage, path: Sdf.Path):
        ...
        deformable_targets = (
            DeformableSchemaChecker.physxParticleCloth_paths
        ...
        if actor_paths[slots[0]] in deformable_targets:
            if (
                actor_paths[slots[0]] in DeformableSchemaChecker.physxParticleCloth_paths
                or actor_paths[slots[0]] in DeformableSchemaChecker.physxDeformableSurface_paths
            ):
        ...
        if actor_paths[slots[1]] in deformable_targets:
            ...
            if actor_paths[slots[0]] in DeformableSchemaChecker.physxParticleCloth_paths:
                DeformableSchemaChecker.convert_DeformableTriMeshRigidAttachment(
                    prim, deformable_prim, slots[0], xformable
                )
                ...
                DeformableSchemaChecker.convert_DeformableBodyRigidAttachment(
                    prim, deformable_prim, slots[0], xformable
                )
            else:
                return
